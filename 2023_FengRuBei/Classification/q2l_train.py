    ...
    if args.inference:
        ...
        for epoch in range(1, args.epochs + 1):
            logger.info(' ===================== Epoch {epoch}  ======================'
                        .format(epoch=epoch))
            TrainLoss, mAP = train(train_loader, model, criterion, optimizer, args, logger)
            writer.add_scalar('small TrainLoss lr=0.00000005 0-850',TrainLoss,epoch)
            writer.add_scalar('small TrainmAP lr=0.00000005 0-850',mAP,epoch)
            logger.info(' * train mAP {mAP:.1f}'
                        .format(mAP=mAP))
            _, mAP = validate(val_loader, model, criterion, args, logger, best_map)
            writer.add_scalar('small mAP lr=0.00000005 0-850',mAP,epoch)
            logger.info(' * mAP {mAP:.1f}'
                        .format(mAP=mAP))
